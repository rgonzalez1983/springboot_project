...
public class CityInterceptor extends GenericInterceptor implements HandlerInterceptor {
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        RequestWrapper requestWrapper = new RequestWrapper(request);
        String requestData = requestWrapper.getBody();
        String uri = requestWrapper.getRequestURI();
        CityEntity data = new Gson().fromJson(requestData, CityEntity.class);
        ...
        if(!StringUtils.hasLength(data.getName()) || data.getCountry() == 0
                || !StringUtils.hasLength(String.valueOf(data.getCountry()))){
            isError = true;
